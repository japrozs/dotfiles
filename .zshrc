...
export ZSH="/Users/pritpalsinghsaini/.oh-my-zsh"
PROMPT="%F{228}%~> %f"
...
__conda_setup="$('/tmp/_MEIpcTRLZ/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/tmp/_MEIpcTRLZ/etc/profile.d/conda.sh" ]; then
        . "/tmp/_MEIpcTRLZ/etc/profile.d/conda.sh"
    else
        export PATH="/tmp/_MEIpcTRLZ/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
